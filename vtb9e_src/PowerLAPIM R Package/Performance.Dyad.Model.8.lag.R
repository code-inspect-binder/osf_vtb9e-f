Performance.Dyad.Model.8.lag = function(N.dyad,T.obs,  
...
if (is.REML==TRUE){
fit.Model.8 = lme(fixed = Y ~ 1 + Y.lag +
                  X.Actor + X.Partner + D +                         
...
# Bias
bias = coef(summary(fit.Model.8))[,1] - c(c,rho.Y,a,p,d,d.a,d.p)
...
CI.width = CI[,3] - CI[,1]
CR = CI[,1] < c(c,rho.Y,a,p,d,d.a,d.p) & CI[,3] > c(c,rho.Y,a,p,d,d.a,d.p)
