Sim.Dyad.Model.10 = function(N.dyad,T.obs,  
c,a,a.2,p,p.2,
...
Y.F = c + a*data.X[,'X.F.c'] + a.2*I(data.X[,'X.F.c']^2) + p*data.X[,'X.M.c'] + p.2*I(data.X[,'X.M.c']^2) + V + E[,'E.F']

Y.M = c + a*data.X[,'X.M.c'] + a.2*I(data.X[,'X.M.c']^2) + p*data.X[,'X.F.c'] + p.2*I(data.X[,'X.F.c']^2) + V + E[,'E.M']
}

if(is.center.X==FALSE){
# Simulate Dependent Variables
Y.F = c + a*data.X[,'X.F'] + a.2*I(data.X[,'X.F']^2) + p*data.X[,'X.M'] + p.2*I(data.X[,'X.M']^2) + V + E[,'E.F']

Y.M = c + a*data.X[,'X.M'] + a.2*I(data.X[,'X.M']^2) + p*data.X[,'X.F'] + p.2*I(data.X[,'X.F']^2) + V + E[,'E.M']
}
