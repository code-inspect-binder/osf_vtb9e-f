Sim.Dyad.Model.2 = function(N.dyad,T.obs,  
c,a,p,
...
Y.F = c + a*data.X[,'X.F.c'] + p*data.X[,'X.M.c'] + V + E[,'E.F']

Y.M = c + a*data.X[,'X.M.c'] + p*data.X[,'X.F.c'] + V + E[,'E.M']
}

if(is.center.X==FALSE){
# Simulate Dependent Variables
Y.F = c + a*data.X[,'X.F'] + p*data.X[,'X.M'] + V + E[,'E.F']

Y.M = c + a*data.X[,'X.M'] + p*data.X[,'X.F'] + V + E[,'E.M']
}
