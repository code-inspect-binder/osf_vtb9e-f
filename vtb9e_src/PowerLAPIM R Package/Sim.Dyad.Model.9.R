Sim.Dyad.Model.9 = function(N.dyad,T.obs,  
...
Y.F = c.F + a.FF*data.X[,'X.F.c'] + a.FF2*I(data.X[,'X.F.c']^2) + p.MF*data.X[,'X.M.c'] + p.MF2*I(data.X[,'X.M.c']^2) + V[,'V.F'] + E[,'E.F']

Y.M = c.M + a.MM*data.X[,'X.M.c'] + a.MM2*I(data.X[,'X.M.c']^2) + p.FM*data.X[,'X.F.c'] + p.FM2*I(data.X[,'X.F.c']^2) + V[,'V.M'] + E[,'E.M']
}

if(is.center.X==FALSE){
# Simulate Dependent Variables
Y.F = c.F + a.FF*data.X[,'X.F'] + a.FF2*I(data.X[,'X.F']^2) + p.MF*data.X[,'X.M'] + p.MF2*I(data.X[,'X.M']^2) + V[,'V.F'] + E[,'E.F']

Y.M = c.M + a.MM*data.X[,'X.M'] + a.MM2*I(data.X[,'X.M']^2) + p.FM*data.X[,'X.F'] + p.FM2*I(data.X[,'X.F']^2) + V[,'V.M'] + E[,'E.M']
}
