Sim.Dyad.Model.9.lag = function(N.dyad,T.obs,  
...
if (is.center.X == TRUE){
# Initialized values
Y.F[T.obs.i[1]] = c.F + a.FF*data.X[T.obs.i[1],'X.F.c'] + a.FF2*I(data.X[T.obs.i[1],'X.F.c']^2) + p.MF*data.X[T.obs.i[1],'X.M.c'] + p.MF2*I(data.X[T.obs.i[1],'X.M.c']^2) + V[T.obs.i[1],'V.F'] + E[T.obs.i[1],'E.F']

Y.M[T.obs.i[1]] = c.M + a.MM*data.X[T.obs.i[1],'X.M.c'] + a.MM2*I(data.X[T.obs.i[1],'X.M.c']^2) + p.FM*data.X[T.obs.i[1],'X.F.c'] + p.FM2*I(data.X[T.obs.i[1],'X.F.c']^2) + V[T.obs.i[1],'V.M'] + E[T.obs.i[1],'E.M']

for (t in T.obs.i[-1]){
# Simulate Dependent Variables
Y.F[t] = c.F + rho.YF*Y.F[t-1] + a.FF*data.X[t,'X.F.c'] + a.FF2*I(data.X[t,'X.F.c']^2) + p.MF*data.X[t,'X.M.c'] + p.MF2*I(data.X[t,'X.M.c']^2) + V[t,'V.F'] + E[t,'E.F']

Y.M[t] = c.M + rho.YM*Y.M[t-1] + a.MM*data.X[t,'X.M.c'] + a.MM2*I(data.X[t,'X.M.c']^2) + p.FM*data.X[t,'X.F.c'] + p.FM2*I(data.X[t,'X.F.c']^2) + V[t,'V.M'] + E[t,'E.M']
}}

if (is.center.X == FALSE){
# Initialized values
Y.F[T.obs.i[1]] = c.F + a.FF*data.X[T.obs.i[1],'X.F'] + a.FF2*I(data.X[T.obs.i[1],'X.F']^2) + p.MF*data.X[T.obs.i[1],'X.M'] + p.MF2*I(data.X[T.obs.i[1],'X.M']^2) + V[T.obs.i[1],'V.F'] + E[T.obs.i[1],'E.F']

Y.M[T.obs.i[1]] = c.M + a.MM*data.X[T.obs.i[1],'X.M'] + a.MM2*I(data.X[T.obs.i[1],'X.M']^2) + p.FM*data.X[T.obs.i[1],'X.F.c'] + p.FM2*I(data.X[T.obs.i[1],'X.F']^2) + V[T.obs.i[1],'V.M'] + E[T.obs.i[1],'E.M']

for (t in T.obs.i[-1]){
# Simulate Dependent Variables
Y.F[t] = c.F + rho.YF*Y.F[t-1] + a.FF*data.X[t,'X.F'] + a.FF2*I(data.X[t,'X.F']^2) + p.MF*data.X[t,'X.M'] + p.MF2*I(data.X[t,'X.M']^2) + V[t,'V.F'] + E[t,'E.F']

Y.M[t] = c.M + rho.YM*Y.M[t-1] + a.MM*data.X[t,'X.M'] + a.MM2*I(data.X[t,'X.M']^2) + p.FM*data.X[t,'X.F'] + p.FM2*I(data.X[t,'X.F']^2) + V[t,'V.M'] + E[t,'E.M']
}}}
