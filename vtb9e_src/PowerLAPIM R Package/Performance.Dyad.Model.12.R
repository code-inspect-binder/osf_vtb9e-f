Performance.Dyad.Model.12 = function(N0.dyad,N1.dyad,T.obs,  
...
if (is.REML==TRUE){
fit.Model.12 = lme(fixed = Y ~ 1 + Z + X.Actor + X.Actor.2 + X.Actor.Z + X.Actor.2.Z + 
...
# Bias
bias = coef(summary(fit.Model.12))[,1] - c(c0,c1,a0,a02,a1,a12,p0,p02,p1,p12)
...
CI.width = CI[,3] - CI[,1]
CR = CI[,1] < c(c0,c1,a0,a02,a1,a12,p0,p02,p1,p12) & CI[,3] > c(c0,c1,a0,a02,a1,a12,p0,p02,p1,p12)
