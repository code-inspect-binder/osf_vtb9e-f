Sim.Dyad.Model.2.lag = function(N.dyad,T.obs,  
c,rho.Y,a,p,
...
if (is.center.X == TRUE){
# Initialized values
Y.F[T.obs.i[1]] = c + a*data.X[T.obs.i[1],'X.F.c'] + p*data.X[T.obs.i[1],'X.M.c'] + V[T.obs.i[1]] + E[T.obs.i[1],'E.F']

Y.M[T.obs.i[1]] = c + a*data.X[T.obs.i[1],'X.M.c'] + p*data.X[T.obs.i[1],'X.F.c'] + V[T.obs.i[1]] + E[T.obs.i[1],'E.M']

for (t in T.obs.i[-1]){
# Simulate Dependent Variables
Y.F[t] = c + rho.Y*Y.F[t-1] + a*data.X[t,'X.F.c'] + p*data.X[t,'X.M.c'] + V[t] + E[t,'E.F']

Y.M[t] = c + rho.Y*Y.M[t-1] + a*data.X[t,'X.M.c'] + p*data.X[t,'X.F.c'] + V[t] + E[t,'E.M']
}}

if (is.center.X == FALSE){
# Initialized values
Y.F[T.obs.i[1]] = c + a*data.X[T.obs.i[1],'X.F'] + p*data.X[T.obs.i[1],'X.M'] + V[T.obs.i[1]] + E[T.obs.i[1],'E.F']

Y.M[T.obs.i[1]] = c + a*data.X[T.obs.i[1],'X.M'] + p*data.X[T.obs.i[1],'X.F'] + V[T.obs.i[1]] + E[T.obs.i[1],'E.M']

for (t in T.obs.i[-1]){
# Simulate Dependent Variables
Y.F[t] = c + rho.Y*Y.F[t-1] + a*data.X[t,'X.F'] + p*data.X[t,'X.M'] + V[t] + E[t,'E.F']

Y.M[t] = c + rho.Y*Y.M[t-1] + a*data.X[t,'X.M'] + p*data.X[t,'X.F'] + V[t] + E[t,'E.M']
}}}
