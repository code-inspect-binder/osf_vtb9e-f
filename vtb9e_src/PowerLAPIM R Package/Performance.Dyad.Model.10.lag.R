Performance.Dyad.Model.10.lag = function(N.dyad,T.obs,  
c,rho.Y,a,a.2,p,p.2,
...
if (is.REML==TRUE){
fit.Model.10 = lme(fixed = Y ~ 1 + Y.lag + 
                  X.Actor + X.Actor.2 + X.Partner + X.Partner.2,
...
# Fixed Effects
# Estimated values
coef = coef(summary(fit.Model.10))[,1]

# Bias
bias = coef(summary(fit.Model.10))[,1] - c(c,rho.Y,a,a.2,p,p.2)

# Power
power = coef(summary(fit.Model.10))[,5]<alpha

# Coverage rate
CI = intervals(fit.Model.10, level = 1-alpha, which = "fixed")$fixed
CI.width = CI[,3] - CI[,1]
CR = CI[,1] < c(c,rho.Y,a,a.2,p,p.2) & CI[,3] > c(c,rho.Y,a,a.2,p,p.2)
...
rho.eps.FM.hat = as.numeric(coef(fit.Model.10$modelStruct$corStruct,unconstrained=FALSE)) 
