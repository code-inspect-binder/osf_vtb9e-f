Performance.Dyad.Model.lag = function(N.dyad,T.obs,  
...
data = Sim.Dyad.Model.lag(N.dyad,T.obs,  
...
if (is.REML==FALSE){
fit.Model.1 = lme(fixed = Y ~ -1 + Female +  
                  Female:X.Actor + Female:X.Partner + 
                  Male + Male + Male:X.Actor + Male:X.Partner,
                  random = ~ -1 + Female + Male |dyad.ID, 
                  correlation = corCompSymm(form = ~1|dyad.ID/Obs),
                  weights = varIdent(form = ~1|Gender),
                  data = data, na.action=na.omit,
...
# Bias
bias = coef(summary(fit.Model.1))[,1] - c(c.F,c.M,a.FF,p.MF,a.MM,p.FM)
...
CI.width = CI[,3] - CI[,1]
CR = CI[,1] < c(c.F,c.M,a.FF,p.MF,a.MM,p.FM) & CI[,3] > c(c.F,c.M,a.FF,p.MF,a.MM,p.FM)
