Performance.Dyad.Model.9.lag = function(N.dyad,T.obs,  
...
if (is.REML==TRUE){
fit.Model.9 = lme(fixed = Y ~ -1 + Female + Female:Y.lag + 
...
# Bias
bias = coef(summary(fit.Model.9))[,1] - c(c.F,c.M,rho.YF,a.FF,a.FF2,p.MF,p.MF2,rho.YM,a.MM,a.MM2,p.FM,p.FM2)
...
CI.width = CI[,3] - CI[,1]
CR = CI[,1] < c(c.F,c.M,rho.YF,a.FF,a.FF2,p.MF,p.MF2,rho.YM,a.MM,a.MM2,p.FM,p.FM2) & CI[,3] > c(c.F,c.M,rho.YF,a.FF,a.FF2,p.MF,p.MF2,rho.YM,a.MM,a.MM2,p.FM,p.FM2)
