Performance.Dyad.Model.10 = function(N.dyad,T.obs,  
c,a,a.2,p,p.2,
...
if (is.REML==TRUE){
fit.Model.10 = lme(fixed = Y ~ 1 + X.Actor + X.Actor.2 + X.Partner + X.Partner.2,
...
# Bias
bias = coef(summary(fit.Model.10))[,1] - c(c,a,a.2,p,p.2)
...
CI.width = CI[,3] - CI[,1]
CR = CI[,1] < c(c,a,a.2,p,p.2) & CI[,3] > c(c,a,a.2,p,p.2)
