Sim.Dyad.Model.lag = function(N.dyad,T.obs,  
c.F,c.M,rho.YF,rho.YM,
...
Y.total = function(c.F,c.M,rho.YF,rho.YM,data.X,V,E,is.center.X,i){
T.obs.i = which(data.X$ID==i) 
Y.F = list()
Y.M = list()

# Initialized values
Y.F[[1]] = c.F + a.FF*data.X[T.obs.i[1],'X.F.c'] + V[T.obs.i[1],'V.F'] + E[T.obs.i[1],'E.F']

Y.M[[1]] = c.M + a.MM*data.X[T.obs.i[1],'X.M.c'] + V[T.obs.i[1],'V.M'] + E[T.obs.i[1],'E.M']

for (t in T.obs.i[-1]){
# Simulate Dependent Variables
Y.F[[which(T.obs.i==t)]] = c.F + rho.YF*Y.F[[which(T.obs.i==t-1)]] + V[t,'V.F'] + E[t,'E.F']

Y.M[[which(T.obs.i==t)]] = c.M + rho.YM*Y.M[[which(T.obs.i==t-1)]] + V[t,'V.M'] + E[t,'E.M']
}
return(cbind(unlist(Y.F),unlist(Y.M)))}

n.ID = unique(data.X$ID)
data.Y = future_lapply(n.ID, function(i) Y.total(c.F,c.M,rho.YF,rho.YM,data.X,V,E,is.center.X,i),future.seed = 0xBEEF)
data.Y = matrix(unlist(data.Y), ncol=2, byrow=T)
